...
public class InTeamZone {
	
	public static Tournamental plugin;
	public InTeamZone(Tournamental instance){
		 plugin = instance;
		}
	...
	public static boolean query(Location l){
		...
		while (counter < size) {
			String zone = zone_list.get(counter);
				Vector pos1 = plugin.getZonesConfig().getVector("Zones." + zone + ".main.team.pos1");
				Vector pos2 = plugin.getZonesConfig().getVector("Zones." + zone + ".main.team.pos2");
